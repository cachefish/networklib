...
TcpConnection::TcpConnection(int sockfd):sockfd_(sockfd),sockIO_(sockfd)
...
{
    char str[100];
    snprintf(str,sizeof(str),"%s:%d->%s:%d",localAddr_.ip().c_str(),
    localAddr_.port(),peerAddr_.ip().c_str(),peerAddr_.port());
